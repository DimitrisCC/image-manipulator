...
// Bonus!
//
// Block iterator below used to exhaustively read the sequence of
// data in a block.
...
namespace imaging{
		...
		Block * Block::copyFromImage(Image & src, Image::channel_t channel, const ivec2 & pos, const size_t & size){
			...
			Component * temp_copy = new Component[copy_size]; //MA
		...
		void Block::setErrorMargin(Component err) { error_margin = err; } //parapanw elegxoi??? px i diafora tous dn mporei na einai megaliteri apo 254 etsi?
		...
		Block::iterator::iterator(Block *p_data)
			: block_ptr(p_data), iter(0), block_size(p_data->size) {}	   // Ah..  a practical one. Should start at element 0.

		Block::iterator::iterator(Block::iterator& src)		   // copy constructor
			: block_ptr(src.block_ptr), iter(src.iter), block_size(src.block_size) {}
		...
		Block::iterator::iterator(Block *p_data, const size_t position)
			: block_ptr(p_data), iter(position), block_size(p_data->size){ }
